...
def play_innings(batsman_name, bowler_name, human_is_batsman, target=None, max_balls=None):
    ...
    while True:
        ...
        if human_is_batsman:
            bat = get_input(f"{batsman_name}, enter runs (0-6): ", VALID_RUNS)
            bowl = computer_choice(VALID_RUNS)
        else:
            bat = computer_choice(VALID_RUNS)
            bowl = get_input(f"{bowler_name}, enter bowl (0-6): ", VALID_RUNS)
        ...
        if bat == 0:
            ...
            if zero_count == 3:
                print("OUT! Batsman used 0 three times.")
                break
...
def toss():
    ...
    user_num = get_input("Enter toss number (0-6): ", VALID_RUNS)
